...
namespace Scoz.Func {
    public class MyMath : MonoBehaviour {
        public static float Round(float _num, int _decimalPlaces) {
            float factor = Mathf.Pow(10f, _decimalPlaces);
            return Mathf.Round(_num * factor) / factor;
        }
        /// <summary>
        /// 取得正1或負1
        /// </summary>
        ...
        /// 無條件捨去至小數X位數
        /// </summary>
        public static decimal RoundDown_Decimal(decimal _num, int _decimalPlaces) {
            if (_decimalPlaces < 0) throw new System.ArgumentOutOfRangeException(nameof(_decimalPlaces));
            decimal power = 1m;
            for (int i = 0; i < _decimalPlaces; i++) power *= 10m;
            return System.Math.Floor(_num * power) / power;
        }
