...
namespace Scoz.Func {
    public static class ComponentManage {
        ...
        public static T CopySkill<T>(this Component comp, T other) where T : Component {
            ...
            foreach (var finfo in finfos) {
                if (Array.Exists<string>(ExcluedSkillFields, element => element == finfo.Name))
                    continue;
                finfo.SetValue(comp, finfo.GetValue(other));
                //WriteLog.Log(finfo.GetValue(comp));
            }
            return comp as T;
        }
        ...
        public static T[] GetComponentsInChildrenExcludeSelf<T>(this Component comp, bool includeInactive) where T : Component {
            if (comp == null)
                return null;
            T[] comList = comp.GetComponentsInChildren<T>(includeInactive);
